...
var BASE_URL = 'https://bws.testnet.keoken.io:3232/bws/api';
...
API.prototype.getTxHistory = function(opts, cb) {
  ...
  if (opts) {
    ...
    if (opts.filterByAssetId) args.push('filterByAssetId=' + opts.filterByAssetId);
  }
  var qs = '';
  if (args.length > 0) {
    qs = '?' + args.join('&');
...
API.prototype.getKeokenBalance = function (walletId, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkState(this.credentials.sharedEncryptingKey);
  var self = this;
  ...
  self._doGetRequest('/v1/keobalance/' + walletId, cb);
};
 ...
 */
API.prototype.getKeokenBalance = function (opts, cb) {
  // $.checkState(this.credentials && this.credentials.isComplete());
  // $.checkState(this.credentials.sharedEncryptingKey);
  // $.checkArgument(opts);
  var self = this;
  self._doGetRequest('/v1/keobalance/' + this.id, function (err, balance) {
    if (err) {
      return cb(err, 0);
    } else {
...
/**
 * Get all keoken assets.
 * @param cb Callback function, returns an array of assets.
 */
API.prototype.getKeokenAssets = function (cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkState(this.credentials.sharedEncryptingKey);
  var self = this;
  self._doGetRequest('/v1/keoassets', function (err, assets) {
    if (err) {
      return cb(err, 0);
    } else {
...
/**
 * Associate an asset to a specific wallet, even if zero tokens are owned,
 * and no transactions made yet.
 * @param walletId Unique identifier for the wallet.
 * @param assetId Unique identifier for the asset, as retrieved from getKeokenAssets method.
 * @param cb Callback function, returns an array of assets.
 */
API.prototype.bindAssetToWallet = function(walletId, assetId, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkState(this.credentials.sharedEncryptingKey);
  if (!_.isString(walletId)) {
    return cb("Wallet id is not a string", 0);
  }
  if (!_.isInteger(assetId)) {
    return cb("Asset id is not an integer", 0);
  }
  var self = this;
  self._doPutRequest('/v2/wallets/' + walletId + '/assets/' + assetId, {}, function(err, result) {
    return err? cb(err, 0) : cb(null, result);
  });
}

/**
 * Get assets bound to a specific wallet
 * @param walletId Unique identifier for the wallet
 * @param cb Callback function, returns an array of assets
 */
API.prototype.getKeokenAssetsByWallet = function (walletId, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkState(this.credentials.sharedEncryptingKey);
  if (!_.isString(walletId)) {
    return cb("Wallet id is not a string", 0);
  ...
  if (!_.isFunction(cb)) {
    return cb("Callback is not a function", 0);
  }
  var self = this;
  self._doGetRequest('/v2/wallets/' + walletId + '/assets', function (err, assets) {
    return err? cb(err, 0) : cb(null, assets);
  });
};
